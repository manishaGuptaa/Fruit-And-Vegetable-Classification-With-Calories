...
model = load_model(r"C:\Users\dell\Desktop\dekstop_folder\projects\Fruit_Vegetable_Recognition_With_Calories\FV.h5")
...
def run():
    ...
    if img_file is not None:
        ...
        save_image_path = "C:\\Users\\dell\\Desktop\\dekstop_folder\\projects\\Fruit_Vegetable_Recognition_With_Calories\\upload_imaages\\"+img_file.name
        ...
        if img_file is not None:
            ...
            st.title("Predicted")   
            st.header(result)
            pre="Predicted : "+result    
            cal = fetch_calories(result)
            if cal:
                st.title("Calories in 100gm")
                st.header(cal)
